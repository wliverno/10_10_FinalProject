...
heating_duty_for_sensible_heating = sum(distillation_feed(1:9).*((Materials_Properties((1:9),1))'/1e3).*(Materials_Properties(1:9,3)').*(458-distillation_feed(10)));   %  in KJ.  Heat used to heat distillation feed from its temperature to 458 degrees Kelvin
...
cooling_duty_for_sensible_cooling = sum(distillation_feed([1,2,9]).*(((Materials_Properties([1,2,9],1)))'/1e3).*(Materials_Properties([1,2,9],3)').*(298-458)); %  in KJ.  Heat removed during the cooling of the distillate from 458 to 298 degrees Kelvin 
...
%
distillation_period = (18*(Distillation_overhead_product(1)+Distillation_overhead_product(2)))/3600;   	% in hours
...
%   Labor Cost 
distillation_labor_cost = Labor_unit_cost * (distillation_period + DistillationTurn);    % in $
%
